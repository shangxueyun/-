  ...
  export default {
    ...
    methods: {
      ...
      getDataList (num) {
        ...
        this.$http({
          url: this.$http.adornUrl('/bim/problemType/qualityRootNodePage'),
          method: 'post',
      ...
      load(tree, treeNode, resolve) {
        if(tree.hasChildren == 1){
            this.$http({
              url: this.$http.adornUrl('/bim/problemType/qualityChildrenNodes/'+tree.id),
              method: 'POST',
      ...
      addOrUpdateHandle (num) {
        ...
        this.$nextTick(() => {
          this.$refs.addOrUpdate.init(id.id,'/bim/problemType/qualitySave')
        })
      },
      updatenameHandle(){
        ...
        this.$nextTick(() => {
          this.$refs.updatename.init(rows.name,rows.id,'/bim/problemType/qualityUpdate')
        })
      ...
      subordinateHandle(){
        ...
        this.$nextTick(() => {
          this.$refs.updatesa.init(rows.type,rows.id,'/bim/problemType/qualitySave')
        })
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定进行[${ids.id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/bim/problemType/qualityDelete'+'/'+ids.id),
            method: 'post',

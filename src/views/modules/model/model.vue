<template>
  <div>
    ...
    <div class="entitymodel">
      ...
      <div class="treelist" v-show="temphide">
        <!-- <el-tree
          :data="datamenu"
          show-checkbox
          node-key="id"
  ...
  export default {
    ...
    data () {
      return {
        ...
        modelDataId: '',
        temphide: false,
        // glb:'./static/js/bim/cube.glb'

      }
    },
    mounted () {
      EngineInit('viewer', true, '../../../../config/cube.glb')
      var modelList = new Array(this.$route.query.url)
...
//              ResetClip();
      ...
      loadNode1 (node, resolve) {
        if (node.level === 0) {
          this.$http({
            url: this.$http.adornUrl('/bim/tree/getPropertyTypeItemList/' + this.$route.query.type),
        ...
        if (node.level == 1) {
          ...
          this.$http({
            url: this.$http.adornUrl('/bim/tree/getPropertyTypeData/' + this.$route.query.code),
            method: 'get',
            params: this.$http.adornParams({
              // 'MileageID': 6
            })

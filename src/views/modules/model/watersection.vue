<template>
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <!-- <el-form-item>
        <el-input v-model="dataForm.key" placeholder="请输入专业分组名称" clearable></el-input>
      </el-form-item> -->
      <el-form-item>
        <!-- <el-button @click="getDataList()">查询</el-button> -->

        <!-- <el-button v-if="isAuth('bim:modelMajor:save')" type="primary" @click="addOrUpdateHandle()">新增流水段</el-button> -->
      </el-form-item>
      <!-- 单体分组操作 -->
      <el-form-item > 
    ...
    <el-table
      ...
      <el-table-column
        ...
        width="220">
              <template slot-scope="scope">
                        <el-table
                            :data="scope.row.pipelineViewList"
                                fit
                                ref="modelTable"
            ...
            <template slot-scope="scope">
            <div v-if="scope.row.modelId == null"><img  style="width:20px;height:20px;" src="~@/assets/img/succLj.png" alt=""></div>
            <div v-else><img style="width:20px;height:20px;" src="~@/assets/img/glmodel.png" alt=""></div>
    ...
    <waterguanlian v-if="waterguanlianVisible" ref="waterguanlianlong" @refreshDataList="getDataList"></waterguanlian>
  </div>
</template>

<script>
  import AddOrUpdate from './entity/major-add-or-update'
  import AddOrWater from './entity/water-updates'
  
  import TableList from './entity/entitymodel-table-list'
  import AddMonomer from './entity/monomer-add'

  import WaterUpdate from './entity/water-updateedit'
  import ModelUpload from './entity/model-upload'
  import waterguanlian from './entity/water-guanlian'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/bim/pipelineView/listPipelineView'),
          method: 'post',
      ...
      handleRowClickModel(row, column, event){
          ...
          this.modeleone=true,
          this.modeledis= false,
          this.modeletwo=false,
          this.modelethree=false
          this.dataListSelections = row
          console.log(row)
          if(row.modelId == null || row.modelId == ''){
              this.modeledis = false
          }else{
              this.modeledis = true
          }
      ...
      waterupdateclick(){
        ...
        this.$nextTick(() => {
          this.$refs.waterUpdate.init(rows.id,rows.name)
        })
      },

      waterupdateclickguanlian(){
        console.log("this.dataListSelections",this.dataListSelections)
        var rows = this.dataListSelections
        this.waterguanlianVisible = true
        this.$nextTick(() => {
          this.$refs.waterguanlianlong.init(rows.modelMonomerId,rows.id)
        })
      },
      ...
      delmodel(){
        var rows = this.dataListSelections
            this.$confirm('该流水段删除后将无法恢复，是否删除？', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                 this.$http({
                    url: this.$http.adornUrl('/bim/pipelineView/delete'),
                    method: 'post',
                    data: this.$http.adornData({
                        "id":rows.id
                    })

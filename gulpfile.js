...
// cube文件
gulp.task('concat:cube', ['replace:version'], function () {
  return gulp.src([`${versionPath}/static/config/cube.glb`])
    .pipe(gulp.dest(`${distPath}/config/`))
});
...
gulp.task('default', ['clean'], function () {
  ...
  // 开始打包编译
  gulp.start(['build', 'create:versionCatalog', 'replace:cdnUrl', 'replace:version', 'concat:config', 'concat:cube'], function () {
    // 清除, 编译 / 处理项目中产生的文件
